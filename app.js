...
function setupForms() {
    ...
    // Formulario de reuniones de área
    document.getElementById('form-reuniones').addEventListener('submit', function(e) {
        e.preventDefault();
        mostrarReunionesArea();
    });

    // Formulario de atención a acudientes
    document.getElementById('form-atencion').addEventListener('submit', function(e) {
        e.preventDefault();
        mostrarAtencionAcudientes();
    });
}
...
// ============================================================================
// NUEVAS FUNCIONALIDADES: REUNIONES DE ÁREA Y ATENCIÓN A ACUDIENTES
// ============================================================================

// Poblar select de docentes para atención a acudientes
function populateDocentesAtencion() {
    const selectDocente = document.getElementById('atencion-docente');
    const docentes = ATENCION_ACUDIENTES.map(a => a.docente).sort();
    
    docentes.forEach(docente => {
        selectDocente.add(new Option(docente, docente));
    });
}

// Mostrar reuniones de área
function mostrarReunionesArea() {
    const diaFiltro = document.getElementById('reuniones-dia').value;
    
    let reuniones = REUNIONES_AREA;
    if (diaFiltro) {
        reuniones = reuniones.filter(r => r.dia === diaFiltro);
    }
    
    const title = diaFiltro 
        ? `Reuniones de Área - ${diaFiltro}`
        : 'Reuniones de Área - Todas';
    ...
    if (reuniones.length === 0) {
        html += `
            <div class="no-results">
                <div class="no-results-icon">📋</div>
                <div class="no-results-text">No hay reuniones programadas para este día</div>
            </div>
        `;
    } else {
        // Agrupar por día
        ...
        reuniones.forEach(r => {
            if (!reunionesPorDia[r.dia]) {
                reunionesPorDia[r.dia] = [];
            }
            reunionesPorDia[r.dia].push(r);
        });
        ...
        diasOrdenados.forEach(dia => {
            if (reunionesPorDia[dia]) {
                html += `<div style="margin-top: 24px; margin-bottom: 16px; padding-bottom: 8px; border-bottom: 2px solid var(--primary); font-size: 20px; font-weight: 600; color: var(--primary);">${dia}</div>`;
                
                reunionesPorDia[dia].forEach(reunion => {
                    html += '<div class="result-card">';
                    html += '<div class="result-card-header">';
                    html += `<div class="result-card-title">📚 ${reunion.area}</div>`;
                    html += `<div class="result-card-badge">${reunion.hora}° hora (${reunion.horaInicio} - ${reunion.horaFin})</div>`;
                    html += '</div>';
                    html += '<div class="result-card-body">';
                    html += `<div class="result-item">
                                <span class="result-label">Docentes</span>
                                <span class="result-value">${reunion.docentes.join(', ')}</span>
                            </div>`;
                    html += `<div class="result-item">
                                <span class="result-label">Lugar</span>
                                <span class="result-value">${reunion.lugar}</span>
                            </div>`;
                    html += '</div>';
                    html += '</div>';
                });
    ...
    html += '</div>';
    mostrarResultados(title, html);
}

// Mostrar atención a acudientes
function mostrarAtencionAcudientes() {
    const docenteFiltro = document.getElementById('atencion-docente').value;
    const diaFiltro = document.getElementById('atencion-dia').value;
    
    let atencion = ATENCION_ACUDIENTES;
    
    if (docenteFiltro) {
        atencion = atencion.filter(a => a.docente === docenteFiltro);
    }
    if (diaFiltro) {
        atencion = atencion.filter(a => a.dia === diaFiltro);
    }
    
    const filtros = [];
    if (docenteFiltro) filtros.push(docenteFiltro);
    if (diaFiltro) filtros.push(diaFiltro);
    
    const title = filtros.length > 0 
        ? `Atención a Acudientes - ${filtros.join(' - ')}`
        : 'Atención a Acudientes - Todos';
    ...
    if (atencion.length === 0) {
        html += `
            <div class="no-results">
                <div class="no-results-icon">👪</div>
                <div class="no-results-text">No se encontraron horarios de atención</div>
            </div>
        `;
    } else {
        // Agrupar por día
        ...
        atencion.forEach(a => {
            if (!atencionPorDia[a.dia]) {
                atencionPorDia[a.dia] = [];
            }
            atencionPorDia[a.dia].push(a);
        });
        ...
        diasOrdenados.forEach(dia => {
            if (atencionPorDia[dia]) {
                html += `<div style="margin-top: 24px; margin-bottom: 16px; padding-bottom: 8px; border-bottom: 2px solid var(--primary); font-size: 20px; font-weight: 600; color: var(--primary);">${dia}</div>`;
                
                // Ordenar por hora dentro del día
                atencionPorDia[dia].sort((a, b) => a.hora - b.hora);
                
                atencionPorDia[dia].forEach(a => {
                    html += '<div class="result-card">';
                    html += '<div class="result-card-header">';
                    html += `<div class="result-card-title">👨‍🏫 ${a.docente}</div>`;
                    html += `<div class="result-card-badge">${a.hora}° hora</div>`;
                    html += '</div>';
                    html += '<div class="result-card-body">';
                    html += `<div class="result-item">
                                <span class="result-label">Horario</span>
                                <span class="result-value">${a.horario}</span>
                            </div>`;
                    html += `<div class="result-item">
                                <span class="result-label">Lugar</span>
                                <span class="result-value">${a.lugar}</span>
                            </div>`;
                    html += '</div>';
                    html += '</div>';
                });
    ...
    html += '</div>';
    html += `<div style="margin-top: 24px; text-align: center; color: var(--text-secondary);">
                Total de horarios: ${atencion.length}
             </div>`;
